/****************************************************************************
** Meta object code from reading C++ file 'receive.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../receive_image/include/receive.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'receive.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_receive_t {
    QByteArrayData data[6];
    char stringdata0[57];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_receive_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_receive_t qt_meta_stringdata_receive = {
    {
QT_MOC_LITERAL(0, 0, 7), // "receive"
QT_MOC_LITERAL(1, 8, 14), // "status_receive"
QT_MOC_LITERAL(2, 23, 0), // ""
QT_MOC_LITERAL(3, 24, 10), // "window_set"
QT_MOC_LITERAL(4, 35, 6), // "ip_set"
QT_MOC_LITERAL(5, 42, 14) // "receive_status"

    },
    "receive\0status_receive\0\0window_set\0"
    "ip_set\0receive_status"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_receive[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   34,    2, 0x06 /* Public */,
       3,    0,   35,    2, 0x06 /* Public */,
       4,    0,   36,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       5,    0,   37,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,

       0        // eod
};

void receive::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        receive *_t = static_cast<receive *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->status_receive(); break;
        case 1: _t->window_set(); break;
        case 2: _t->ip_set(); break;
        case 3: _t->receive_status(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            typedef void (receive::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&receive::status_receive)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (receive::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&receive::window_set)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (receive::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&receive::ip_set)) {
                *result = 2;
                return;
            }
        }
    }
    Q_UNUSED(_a);
}

const QMetaObject receive::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_receive.data,
      qt_meta_data_receive,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *receive::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *receive::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_receive.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt_metacast(_clname);
}

int receive::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void receive::status_receive()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void receive::window_set()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}

// SIGNAL 2
void receive::ip_set()
{
    QMetaObject::activate(this, &staticMetaObject, 2, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
